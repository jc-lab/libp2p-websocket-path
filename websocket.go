...
var WsFmt = mafmt.And(mafmt.TCP, mafmt.Base(P_WS_PATH))

var dialMatcher = mafmt.And(
	mafmt.Or(mafmt.IP, mafmt.DNS),
	mafmt.Base(ma.P_TCP),
	mafmt.Or(
		mafmt.Base(P_WS_PATH),
		mafmt.And(
			mafmt.Or(
				mafmt.And(
					mafmt.Base(ma.P_TLS),
...
func (t *WebsocketTransport) maDial(ctx context.Context, raddr ma.Multiaddr) (manet.Conn, error) {
	...
	localMaAddr, err := manet.FromNetAddr(wscon.LocalAddr())
	if err != nil {
		return nil, err
	}
	mnc := &maconn{
		Conn:       NewConnWithAddr(wscon, isWss, remoteAddr),
		localAddr:  localMaAddr,
		remoteAddr: raddr,
	}
	return mnc, nil
}
